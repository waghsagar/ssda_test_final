...
public class TC03_AdminDashboardReadyOrderDetailsValidationTest extends TestBase {
	...
	public String totalReadyOrders = "";
	...
	@Test(priority=1)
	public void testTotalReadyOrdersDisplayedOnDashboardTab() {
		...
		Assert.assertFalse(adt.getTotalReadyOrders().getText().contentEquals("0"));
		log.info("Ready orders are displayed on Admin Dashboard tab");
		totalReadyOrders = adt.getTotalReadyOrders().getText();
		...
		Assert.assertTrue(aot.getTotalOrdersFound().getText().contains(totalReadyOrders));
		log.info("Total Ready orders displayed on Dashboard tab matches with Ready orders displayed on Orders tab");
	}
	
	@Test(priority=2)
	public void testSearchByDateFilterForReadyOrders() {
		log.info("Validate search by date filter on dashboard tab for Ready orders");
		...
		totalReadyOrders = aot.getTotalOrdersFound().getText();
		...
		Assert.assertTrue(totalReadyOrders.contains(adt.getTotalReadyOrders().getText()));
		log.info("Ready orders by date displayed on Dashboard tab matches with Ready orders by date displayed on Orders tab");
	}
