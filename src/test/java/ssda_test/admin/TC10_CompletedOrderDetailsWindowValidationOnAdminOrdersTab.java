...
public class TC10_CompletedOrderDetailsWindowValidationOnAdminOrdersTab extends TestBase {
	...
	@Test(priority=1)
	public void testCompletedOrderDetailsClickCloseButtonOnOrderDetailsWindow() {
		log.info("Validate Completed order details in Order Details window and click on Close Button");
		...
		for(int j = 0; j < dropdown_list.size(); j++){
			if(dropdown_list.get(j).getText().contains("Completed")){
				log.info("Select Completed from status dropdown to view Completed orders");
