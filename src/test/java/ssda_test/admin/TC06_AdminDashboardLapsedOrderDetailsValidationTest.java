...
public class TC06_AdminDashboardLapsedOrderDetailsValidationTest extends TestBase {
	...
	public String status = "Lapsed";
	public String totalLapsedOrders = "";
	...
	@Test(priority=1)
	public void testTotalLapsedOrdersDisplayedOnDashboardTab() {
		...
		log.info("User clicks on right slide button to view Lapsed orders");
		adt.getOrdersSlideRightButton().click();
		Assert.assertTrue(adt.getTotalLapsedOrders().isDisplayed());
		Assert.assertFalse(adt.getTotalLapsedOrders().getText().contentEquals("0"));
		log.info("Lapsed orders are displayed on Admin Dashboard tab");
		totalLapsedOrders = adt.getTotalLapsedOrders().getText();
		...
		Assert.assertTrue(aot.getTotalOrdersFound().getText().contains(totalLapsedOrders));
		log.info("Total Lapsed orders displayed on Dashboard tab matches with Lapsed orders displayed on Orders tab");
	}
	
	@Test(priority=2)
	public void testSearchByDateFilterForLapsedOrders() {
		log.info("Validate search by date filter on dashboard tab for Lapsed orders");
		...
		String totalLapsedOrders = aot.getTotalOrdersFound().getText();
		...
		Assert.assertTrue(totalLapsedOrders.contains(adt.getTotalLapsedOrders().getText()));
		log.info("Lapsed orders by date displayed on Dashboard tab matches with Lapsed orders by date displayed on Orders tab");
	}
