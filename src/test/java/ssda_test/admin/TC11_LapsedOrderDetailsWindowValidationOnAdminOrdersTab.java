...
public class TC11_LapsedOrderDetailsWindowValidationOnAdminOrdersTab extends TestBase {
	...
	@Test(priority=1)
	public void testLapsedOrderDetailsClickCloseButtonOnOrderDetailsWindow() {
		log.info("Validate Lapsed order details in Order Details window and click on Close Button");
		...
		for(int j = 0; j < dropdown_list.size(); j++){
			if(dropdown_list.get(j).getText().contains("Lapsed")){
				log.info("Select Lapsed from status dropdown to view Lapsed orders");
