...
public class TC07_AdminDashboardCancelledOrderDetailsValidationTest extends TestBase {
	...
	@Test(priority=1)
	public void testTotalCanceledOrdersDisplayedOnDashboardTab() {
		...
		log.info("User clicks on right slide button to view Canceled orders");
		adt.getOrdersSlideRightButton().click();
		Assert.assertTrue(adt.getTotalCanceledOrders().isDisplayed());
		Assert.assertFalse(adt.getTotalCanceledOrders().getText().contentEquals("0"));
		log.info("Canceled orders are displayed on Admin Dashboard tab");
		totalCanceledOrders = adt.getTotalCanceledOrders().getText();
		...
		Assert.assertTrue(aot.getTotalOrdersFound().getText().contains(totalCanceledOrders));
		log.info("Total Canceled orders displayed on Dashboard tab matches with Canceled orders displayed on Orders tab");
	}
	
	@Test(priority=2)
	public void testSearchByDateFilterForCanceledOrders() {
		log.info("Validate search by date filter on dashboard tab for Canceled orders");
		...
		String totalCanceledOrders = aot.getTotalOrdersFound().getText();
		...
		Assert.assertTrue(totalCanceledOrders.contains(adt.getTotalCanceledOrders().getText()));
		log.info("Canceled orders by date displayed on Dashboard tab matches with Canceled orders by date displayed on Orders tab");
	}
